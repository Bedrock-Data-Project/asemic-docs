...
```bash
curl -X GET 'https://api.asemicanalytics.com/api/v1/forgd/datasources/daily' \
  -H 'Authorization: Apikey YOUR_API_KEY' \
  -H 'Accept: application/json'
```
...
**Example 1: Daily Volume by Exchange**

```bash
curl -X POST 'https://api.asemicanalytics.com/api/v1/forgd/charts/submit' \
  -H 'Authorization: Apikey YOUR_API_KEY' \
...
This query shows Revenue data for the top 10 provider/currencies that had the biggest increase in active users (user growth), filtered to only centralized exchanges with significant volume.
...
```bash
curl -X POST 'https://api.asemicanalytics.com/api/v1/forgd/datasources/backfill-userwide/2025-01-01/2025-01-31' \
  -H 'Authorization: Apikey YOUR_API_KEY' \
  -H 'Content-Type: application/json'
```
...
```json
// Top 10 exchanges by volume (default sorting)
{
  "kpi_id": "revenue",
  "column_group_bys": ["provider"],
  "group_by_limit": 10
...
// Markets with highest impact on total volume
{
  "kpi_id": "revenue",
